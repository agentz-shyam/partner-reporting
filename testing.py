...
lastMonthLog = "./test_reporting_scheduler_{}.log".format(format(last_month, '%B'))
...
console = logging.FileHandler("./test_reporting_scheduler_{}.log".format(now.strftime("%B")))
...
class reportingMethod:
    def __init__(self, timezones):
        ...
        self.timezonesBusinessID = [str(id[0]) for id in list(self.cur3.fetchall())]
        logger.info("The available business id for the timezone {} is {}".format(str(self.my_timezone),
                                                                                 ','.join(self.timezonesBusinessID)))

    def business_id(self, date):  
        ...
        if self.timezonesBusinessID:
            for business_id in self.timezonesBusinessID:
                ...
                if row is not None:
                    ...
                    if row[1] in self.timezonesBusinessID:
        ...
        for id in self.cur2.fetchone():
            ...
            try:
                ...
                try:
                    ...
                    logger.info(f"Error while inserting report event data {e}")    
...
for timezones in timezone_list['TimeZone']:
    reporting = reportingMethod(timezones)
    base = dt.datetime.today().date() - dt.timedelta(days=1)
    date_list = [base - dt.timedelta(days=x) for x in range(71)]
    for date in date_list:
        try:
            logger.info("Process initiated for {}".format(date))
            reporting.business_id(date)
        except Exception as e:
            logger.error(e)
