...
lastMonthLog = "./charting_module_{}.log".format(format(last_month, '%B'))
...
console = logging.FileHandler("./charting_module_{}.log".format(now.strftime("%B")))
...
@app.route('/chartvalues', methods=['POST', "GET"])
def report():
    if request.method == "GET":
        ...
        if masterBusinessAgentId:
            db = MySQLdb.connect(host=configFile["analysis"]["host"], user=configFile["analysis"]["user"],
                                 passwd=configFile["analysis"]["pswd"], db=configFile["analysis"]["database"])
            cur = db.cursor()
...
class Chart(object):
    ...
    def __init__(self, startDate, endDate, domainID, businessAgentId):
        ...
        self.db = MySQLdb.connect(host=configFile["analysis"]["host"], user=configFile["analysis"]["user"],
                                  passwd=configFile["analysis"]["pswd"], db=configFile["analysis"]["reportDB"])
    ...
    def statistis(self, ):
        ...
        for self.rows in self.cur.fetchall():
            ...
            conversion.append(int(self.tableData['completedconversion_task']))
            phoneCall.append(int(self.tableData['phonecalls']))
            ...
            one_tasksession.append(int(self.tableData['one_tasksession']))
            offSessions.append(int(self.tableData['sessions_offhours']))
            sessions.append(int(self.tableData['sessions_loaded']))
    ...
    def isInfoTask(self):
        ...
        # self.domainIdApi = "https://" + env + configFile["charting"]["domainAPi"] + self.businessAgentId
        # domainData = requests.get(url=self.domainIdApi, headers=self.headers).json()
        # self.agentDomainId = domainData['domainId']

        self.taskApi = "https://" + env + configFile["charting"]["taskAPi"] + self.agentDomainId + "/tasks"
    ...
    def taskcounts(self, ):
        ...
        if self.isbusinessTask:
            self.cur.execute(util.taskcounts.format(tuple(self.isbusinessTask)), (self.businessAgentId, self.startDate,
                                                                                  self.endDate))
            self.taskCount = dict()
            for self.taskset in self.cur.fetchall():
                ...
                if self.idTaskMap[self.taskset[0]] not in self.taskCount:
                    ...
                    self.taskCount[self.idTaskMap[self.taskset[0]]] = self.taskCount[self.idTaskMap[self.taskset[0]]] + \
                                                                      self.taskset[2]
            result = dict(
                islice(dict(OrderedDict(sorted(self.taskCount.items(), key=lambda val: val[1], reverse=False))).items(),
                       0, 24))
    ...
    def exitcounts(self, ):
        ...
        TaskCount = []
        if self.isbusinessTask:
            self.cur.execute(util.exitcounts.format(tuple(self.isbusinessTask)), (self.businessAgentId, self.startDate,
                                                                                  self.endDate))
            ...
            for self.exittask in self.cur.fetchall():
                ...
                if self.idTaskMap[self.exittask[0]] not in self.finalCount:
                    ...
                    self.finalCount[self.idTaskMap[self.exittask[0]]] = self.finalCount[
                                                                        ...
                                                                        self.exittask[2]
            result = dict(
                islice(dict(OrderedDict(sorted(self.finalCount.items(), key=lambda val: val[1], reverse=False))).items()
                       , 0, 24))
            for tasks, cnt in sorted(result.items(), key=lambda val: val[1], reverse=False):
                TaskName.append(tasks)
                TaskCount.append(cnt)
